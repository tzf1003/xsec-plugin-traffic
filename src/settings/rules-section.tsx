...
export function RulesSection({ host }: { host: PluginHost }) {
  ...
  const save = async () => {
    ...
    try {
      await saveRule(host, submitted); await reload();
  ...
  return <section className="settings-section"><header><div><h2>被动检测规则</h2><p>保存后应用于之后捕获的流量。</p></div></header>
    ...
    <div className="rule-form"><Field label="规则 ID"><input value={draft.rule_id} onInput={(event) => updateDraft({ ...draft, rule_id: event.currentTarget.value })} /></Field><Field label="严重级别"><select value={draft.severity} onChange={(event) => updateDraft({ ...draft, severity: event.currentTarget.value as PassiveRule["severity"] })}><option value="low">low</option><option value="medium">medium</option><option value="high">high</option><option value="critical">critical</option></select></Field><Field className="rule-pattern-field" label="正则表达式"><input value={draft.pattern} onInput={(event) => updateDraft({ ...draft, pattern: event.currentTarget.value })} /></Field><Check checked={draft.enabled} onChange={(enabled) => updateDraft({ ...draft, enabled })}>启用</Check><Button tone="primary" disabled={busy} onClick={() => void save()}>保存</Button></div>
    {loading ? <Spinner label="正在读取被动规则…" /> : null}
    {rules ? <div className="rule-table-wrap"><table className="rule-table"><thead><tr><th>启用</th><th>规则 ID</th><th>严重</th><th>正则</th><th>操作</th></tr></thead><tbody>{rules.map((rule) => <tr key={rule.rule_id}><td><Check checked={rule.enabled} disabled={busy} onChange={(enabled) => void toggle(rule, enabled)}><span className="sr-only">启用 {rule.rule_id}</span></Check></td><td><code>{rule.rule_id}</code></td><td>{rule.severity}</td><td className="rule-pattern" title={rule.pattern}>{rule.pattern}</td><td><Button tone="ghost" onClick={() => updateDraft(rule)}>编辑</Button><Button tone="ghost" icon="trash" onClick={() => setDeleteId(rule.rule_id)}>删除</Button></td></tr>)}</tbody></table></div> : null}
